...
def test_pirads_does_not_enter_the_recurrence_ordering():
    """The withdrawn term, as a test -- see stratified.PIRADS_WITHDRAWN_AFTER.

    It shipped for two validation slots and lost both: skipping it on absence cost
    S3 (0.7851 -> 0.6281), and imputing the training median recovered most of that
    but not all (S4, 0.7438), because on the 20 validation cases that *did* carry
    PI-RADS the term was itself negative (0.8875 -> 0.7500). Both failure modes are
    fenced off by asserting the input has no effect at all: the three bands and the
    absent case must all produce the same months, with only csPCa varying between
    them. Nothing here forbids reinstating the term deliberately -- it forbids
    reinstating it by accident.
    """
    same = {"surgical_pathology_report": SURGICAL}
    ...
    months = [
        _months({**same, "radiology_report": r}, 12.0)
        for r in (silent, RADIOLOGY, RADIOLOGY.replace("PI-RADS: 5", "PI-RADS: 2"))
    ]
    assert months[0] == pytest.approx(months[1]) == pytest.approx(months[2])
