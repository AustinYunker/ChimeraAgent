...
%   - the test-set row in Table 3, once the Sep 10 submission resolves
...
of predicted times matters and its reasoning trace earns nothing. We score each case
with CAPRA-S~\cite{capras} and map it monotonically to months. CAPRA-S takes 12
integer values over 75 training cases, tying 104 of 1130 comparable pairs, each
banking a flat $0.5$. We break ties with the MRI-derived probability of clinically
significant cancer, $\text{risk} = \text{CAPRA-S} + w\,p_{\text{csPCa}}$, with
$w = 0.99$ a deliberate \emph{bound} rather than a tuned weight: the term reorders
within a CAPRA band and never across one, so $p_{\text{csPCa}}$ need only beat a
coin flip, and vanishes where a site emits none. It resolves those ties at $0.663$,
lifting the training C-index from $0.7372$ to $0.7522$.
...
Task~3 fitted survival regression & no &
  the C-index depends only on ordering \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{Validation. S2 differs from S1 only by splitting one Task~2 leaf on ISUP
grade~1; Tasks~1 and~3 returned bit-identical scores, so the change was isolated.}
\label{tab:val}
\centering
\setlength{\tabcolsep}{2pt}
\begin{tabular}{lccc}
